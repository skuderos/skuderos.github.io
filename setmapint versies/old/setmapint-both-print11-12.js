...
for (var i=0; i<waypointlatlng.length; i++) {
   ...
   var lonmarkers = waypointlatlng[i][0];
   var latmarkers = waypointlatlng[i][1];
   var popupText = waypointlatlng[i][2];

    var markerLocation = new L.LatLng(latmarkers, lonmarkers);
    var markeratbounds = new L.Marker(markerLocation, {icon: waypointmarkerpng});
    map.addLayer(markeratbounds);
    console.log('added the markers if all goes well')
